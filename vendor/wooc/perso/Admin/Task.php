 ...
 * $HeadURL: file:///mnt/atl-fs8-data1/svn/webtrees-geneajaubart/trunk/library/WT/Perso/Admin/Task.php $
*/
...
abstract class WT_Perso_Admin_Task{
	...
	public static function isModuleOperational(){
		if(self::$_isModuleOperational == -1){
			self::$_isModuleOperational = array_key_exists('perso_admintasks', Module::getActiveModules());
			if(self::$_isModuleOperational){
				self::$_isModuleOperational = PersoFunctions::doesTableExist('##padmintasks');
			}
		}
	...
	public static function getInstalledTasks() {
		...
		while (($file=readdir($dir))!==false){ 
			if(preg_match('/^([a-zA-Z0-9_]+)\.php$/', $file, $taskname)) {
				...
				$task=new $class();
				Database::prepare('INSERT IGNORE INTO `##padmintasks` (pat_name, pat_status, pat_frequency) VALUES (?, ?, ?)')
	...
	public static function getActiveTasks($sort=true) {		
		$tasks_enabled=Database::prepare(
	...
	public static function deleteTask($task_name){
		...
		Database::prepare('DELETE FROM  `##padmintasks` WHERE pat_name=?')
		->execute(array($task_name));
		Database::prepare("DELETE FROM  `##gedcom_setting` WHERE setting_name LIKE 'PAT_?%'")
		->execute(array($task_name));
	}
