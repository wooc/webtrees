...
class PersoFunctions {
	...
	static public function promptAlert($text){
		echo '<script>';
		echo 'alert("',Filter::escapeHtml($text),'")';
		echo '</script>';
	}
	...
	public static function doesTableExist($table) {
		try {
			Database::prepare("SELECT 1 FROM {$table}")->fetchOne();
			return true;
	...
	*/
	public static function isIsSourcedModuleOperational(){
		if(self::$_isIsSourcedModuleOperational == -1){
			if (Module::getModuleByName('perso_issourced')) {
				self::$_isIsSourcedModuleOperational = 1;
			}
		}
