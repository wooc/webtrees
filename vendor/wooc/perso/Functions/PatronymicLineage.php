...
class PersoFunctions_PatronymicLineage {
	...
	public static function printLineages($surname, $legend, $gedcomid) {
		...
		if(count($indilist)==0) {
			...
			I18N::translate('No individuals with surname %s has been found. Please try another name.', check_NN($surname)),
				'</span></p>';
			return;
		}
	...
	private static function printIndiLineage(WT_Individual $indi_root){
		...
		if($indi_root){
			$dindi = new PersoIndividual($indi_root);
				...
				//Separate the way to display the individual depending if this is its first marriage or not
					...
					if($indi_root->getSex()=='F'){
						if($children && count($children)>0){
							...
							if($spouse) {
								$dspouse = new PersoIndividual($spouse);
								$father_surname=$dspouse->getUnprotectedPrimarySurname();
							}
							$nbNatural=0;
							foreach($children as $child){
								$dchild = new PersoIndividual($child);
