 ...
 * $HeadURL: file:///mnt/atl-fs8-data1/svn/webtrees-geneajaubart/trunk/library/WT/Perso/Certificate.php $
*/
...
class WT_Perso_Certificate extends WT_Media {
	...
	public static function isModuleOperational(){
		if(self::$_isModuleOperational == -1){
			self::$_isModuleOperational = array_key_exists('perso_certificates', Module::getActiveModules());
		}
	...
	public function __construct($data) {
		...
		$xref = PersoFunctions::encryptToSafeBase64($data);
		$gedcom = sprintf(
			'0 @%1$s@ OBJE'.PHP_EOL.
			'1 FILE %2$s',
			$xref, $data
		);
		parent::__construct($xref, $gedcom, '', $WT_TREE->getTreeId());
	...
	static public function getInstance($data, $gedcom_id=$WT_TREE->getTreeId(), $gedcom=null) {
		$certfile = PersoFunctions::decryptFromSafeBase64($data);
		
		//NEED TO CHECK THAT !!!
		if(PersoFunctions::isValidPath($certfile, true)) {
	...
	public function getServerFilename($which='main') {
		$filename = PersoFunctions_Certificates::getRealCertificatesDirectory(). $this->file;
		return PersoFunctions::encodeUtf8ToFileSystem($filename);
	}
	 ...
	 */
	 public function getWatermarkText(){	
		$wmtext = get_module_setting('perso_certificates', 'PC_WM_DEFAULT', I18N::translate('This image is protected under copyright law.'));
	...
	public function linkedIndividuals($link = '_ACT'){
		...
		->execute(array($WT_TREE->getTreeId(), '%_ACT '.$this->file.'%'))->fetchAll();
