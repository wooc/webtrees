 ...
 * @version: p_$Revision: 72 $ $Date: 2013-11-16 12:16:39 +0000 (Sat, 16 Nov 2013) $
 * $HeadURL: file:///mnt/atl-fs8-data1/svn/webtrees-geneajaubart/trunk/library/WT/Perso/Functions/Certificates.php $
 */
...
class PersoFunctions_Certificates {
		...
		if(!isset($_citiesList) || is_null($_citiesList)){
			...
			while($entry = readdir($dir)){
				if($entry!='.' && $entry!='..' && is_dir($certdir.$entry)){
					$_citiesList[]=PersoFunctions::encodeFileSystemToUtf8($entry);
				}
			}
			sort($_citiesList);
		}
	...
	public static function getCertificatesList($selCity){
		...
		$selCity = PersoFunctions::encodeUtf8ToFileSystem($selCity);
		...
		if(is_dir($certdir.$selCity)){
			...
			while($entry = readdir($dir)){
				if($entry!='.' && $entry!='..' && !is_dir($certdir.$entry.'/')){
					$path = PersoFunctions::encodeFileSystemToUtf8($selCity.'/'.$entry);
	...
	public static function getCertificateListBeginWith($city, $contains, $limit= 9999){
		$tabFiles= array();	
		$dirPath=$certdir = self::getRealCertificatesDirectory().PersoFunctions::encodeUtf8ToFileSystem($city).'/';
		...
		if(is_dir($dirPath)){
			...
			while(($entry = readdir($dir)) && $nbCert < $limit){
				if($entry!='.' && $entry!='..' && $entry!='Thumbs.db' &&!is_dir($dirPath.$entry.'/') && stripos($entry, $contains)!== false){
					$tabFiles[]=PersoFunctions::encodeUtf8ToFileSystem($entry);
					$nbCert++;
				}
			}
		}
	...
	public static function printCertificateListBeginWith($city, $contains) {	
		$listCertif=PersoFunctions_Certificates::getCertificateListBeginWith($city, $contains, 10);
