 ...
 * $HeadURL: file:///mnt/atl-fs8-data1/svn/webtrees-geneajaubart/trunk/library/WT/Perso/Functions/Map.php $
 */
...
class FunctionsMap {
	 ...
	 */
	public static function getPlaceHierarchy($gedid = $WT_TREE->getTreeId()){
		$plHierarchy = array();
		...
		if(WT_Perso_Cache::isCached($cacheId)) {
			...
			$plHierarchy['ged_id'] = $WT_TREE->getTreeId();		// This is already taken care of in the session.php
			$plHierarchy['isdefined'] = false;
			$plHierarchy['nblevels'] = 0;
			$plHierarchy['hierarchy'] = null;
			if($placestructure = PersoFunctions_Map::getPlaceHierarchyHeader($WT_TREE->getTreeId())){
				$plHierarchy['isdefined'] = true;
				$plHierarchy['nblevels'] = count($placestructure);
				$plHierarchy['hierarchy'] = $placestructure;
			}
			else{
				list($plHierarchy['nblevels'], $plHierarchy['hierarchy']) = PersoFunctions_Map::getRandomPlaceExample($WT_TREE->getTreeId());
			}
	 ...
	 */
	public static function getPlaceHierarchyHeader($ged_id = $WT_TREE->getTreeId()){
	 ...
	 */
	public static function getRandomPlaceExample($ged_id = $WT_TREE->getTreeId()){
		$randomPlace = null;
		$nbLevels = 0;
	...
	public static function isGeoDispersionModuleOperational(){
		if(self::$_isGeoDispersionOperational == -1){
			self::$_isGeoDispersionOperational = array_key_exists('perso_geodispersion', Module::getActiveModules());
			if(self::$_isGeoDispersionOperational){
				self::$_isGeoDispersionOperational = PersoFunctions::doesTableExist('##pgeodispersion');
			}
		}
	...
	public static function getAvailableGeoDispersionMaps(){
		if (self::$_maps===null) {
			...
			if(is_dir($path)){
				...
				//self::$_maps = array_merge(array('nomap' => I18N::translate('No map')), self::$_maps);
			}
		}
		return self::$_maps;
	}
	 ...
	 */
	public static function getEnabledGeoDispersionMaps($gedid = $WT_TREE->getTreeId()){
		if (self::$_enabledmaps===null) {
			self::$_enabledmaps = Database::prepare(
